local store_lua_fn = require('neoc.core.lib.storefn').store_lua_fn
local M = {}
...
local function merge_attrs(attrs)
  attrs = attrs or ''
  if vim.tbl_islist(attrs) then attrs = table.concat(attrs, ' ') end
...
local function get_exec(cmd, exec)
  local execfn = nil
  if type(exec) == 'function' then execfn = store_lua_fn('commands', cmd, exec) end
...
function M.command(cmd, exec, attrs)
  ...
  if not success then vim.api.nvim_err_writeln(errmsg) end
end
